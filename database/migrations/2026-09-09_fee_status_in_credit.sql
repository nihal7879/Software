-- Two columns in the hours table both read "Active": the student's enrolment
-- Status, and their Fee Status. They mean different things — still on the roll
-- vs. still has paid hours — and the repeated word made the row ambiguous.
--
-- The fee side is renamed to 'In Credit', which reads as the natural opposite of
-- 'Payment Required'. 'Payment Required' and 'Trial' are unchanged.
--
-- The consumed-hours subquery also gains `l.is_deleted = FALSE`, matching the
-- application's own hours queries: a deleted lecture must stop consuming hours
-- here too, or the dashboard totals disagree with every per-student figure.
USE classroom_app;
